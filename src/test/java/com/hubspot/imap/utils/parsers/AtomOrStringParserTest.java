...
public class AtomOrStringParserTest {
  ...
  private static final byte[] QUOTED =
    "\"This is quoted\"".getBytes(StandardCharsets.UTF_8);
  private static final byte[] QUOTED_RESULT = Arrays.copyOfRange(
    QUOTED,
    1,
    QUOTED.length - 1
  );
  private static final byte[] ESCAPED_QUOTES =
    "\"This contains \\\"quotes\\\"\"".getBytes(StandardCharsets.UTF_8);
  private static final byte[] ESCAPED_QUOTES_RESULT = Arrays.copyOfRange(
    ESCAPED_QUOTES,
    1,
    ESCAPED_QUOTES.length - 1
  );
  private static final byte[] LITERAL =
    "{10}\nabcdefghij".getBytes(StandardCharsets.UTF_8);
  private static final byte[] LITERAL_RESULT =
    "abcdefghij".getBytes(StandardCharsets.UTF_8);
  ...
  @Test
  public void testGivenStringWithEscapedQuotesQuotedString_doesReturnStringWithoutSurroundingQuotes()
    throws Exception {
  ...
  @Test
  public void testGivenStringWithLiteral_doesReturnLiteralWithCorrectLength()
    throws Exception {
